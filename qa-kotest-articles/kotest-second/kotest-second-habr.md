...
вариантах тестовых данных и окружений. В этом случае невозможно использовать ограниченный набор данных для тестов, если только тип входных
данных не ограничен парой вариантов - Data-Driver не подходит. В таких случаях необходимо генерировать все тестовые данные случайным образом
либо вообще покрывать абсолютно все возможные значения. Нужно не просто уметь генерировать данные, но и легко менять параметры генерации
типа функций распределения значений, включение в последовательность граничных значений и т.п. Не менее важна возможность повторить
последовательность с прошлого прогона, чтобы воспроизвести ошибку. И еще нужны инструменты, чтобы создавать свои генераторы.
...
`1` - Класс `Codepoint` задает набор символов в виде кодов Unicode. Этот класс используется в `Arb` для генерации строк из определенного
набора символов. В **Kotest** есть встроенные наборы символов в файле `io.kotest.property.arbitrary.codepoints`, здесь определен набор
Unicode цифр (такого набор нет среди встроенных). В дальнейших шагах этот набор будет использован для генерации номеров, которые могут
начинаться на `0`
...
> Вся функциональность генераторов доступна не только для Property-тестов, но и для простых тестов, где нужны случайные значения любых тестовые данные.  
> Для генерации единичного значения используется терминальный метод `Arb.next`
