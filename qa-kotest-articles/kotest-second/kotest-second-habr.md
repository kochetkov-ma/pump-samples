...
Остается задать выражение тегов для запуска ожидаемого набора тестов через системную переменную `-Dkotest.tags=`.  
...
Для номера телефона генерация происходит в 10 раз медленнее, чем при использовании `Arb.string(10,numberCodepoint)` с форматированием.  
У меня **1000** генераций телефона по паттерну -> **294 мс** и 1000 генераций телефона из строки цифр с последующим форматированием -> **32 мс**.  
...
А также же метод должен принимать `null` и генерировать `UUID` типа `UUIDVersion.ANY`.
...
Cемя изначально генерируется случайно, используя внешний порождающий элемент, например, время либо физический процесс. 
