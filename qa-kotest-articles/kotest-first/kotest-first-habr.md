...
Непрерывно нахожусь в поиске набора инструментов для организации автоматизации тестирования в больших командах, который обеспечит:
...
Проанализировав множество вариантов, решил создавать тесты в виде кода, с помощью довольно молодого Фреймворка, написанного на **Kotlin** - **Kotest** (в прошлом _Kotlin Test_).
...
По итогу должен получится емкое руководство — как создать систему, или даже эко-систему, автоматизации функционального тестирования на языке **Kotlin** и платформе **Kotest**.
...
Еще в марте 2020 фреймворк назывался `KotlinTest`, в 4 версии поменял название всех пакетов, потерял совместимость с плагином Idea и был переименован в `Kotest`, а также стал мульти-платформенным.
...
Почему Kotest
------
Множество разработчиков на Kotlin не заморачиваются с выбором тестового фреймворка и продолжают использовать Junit4 или Junit5. 
...
```
Соответсвенно запись `"string" - { }` вызывает функцию расширения `String.minus`, передает внутрь функциональный тип с контекстом `FreeScope` и добавляет в дерево тестов контейнер.  
